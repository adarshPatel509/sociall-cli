		...
		octokit
			...
			.then((res) => {
				...
				for (let i = 0; i < res.data.length; i++) {
					const { reason, subject, repository } = res.data[i];
					...
					const ans = (
						...
						>
							<Text>
								<Text bold>{login}</Text> generated{" "}
								<Text color={"blue"}>
									{feed_reply[type]["emoji"]} {type}
								</Text>{" "}
								in {name} repo of title <Text underline>"{title}"</Text>{" "}
							</Text>
							<Text>
								You got the Notifications because you have{" "}
								<Text underline>{reason}</Text>
							</Text>
						</Box>
					);
					arr.push(ans);
